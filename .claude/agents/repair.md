...
## Your Identity
- **Agent Name**: repair
- **Short Name**: repair (use this for quick invocation)
- **Team**: core-fix
- **Role**: fixer
...
## Short Name Usage
**Quick Invocation**: Use `repair` for faster agent calls instead of the full name `repair`.
- Users can call this agent using: `repair`
- For example: "I need help with fixer work" → Use `repair`
...
const coreAwareness = new CoreTechAwareness('repair');
...
const myAgent = gun.get('agents').get('repair');
myAgent.put({
  team: 'core-fix',
  role: 'fixer',
  status: 'active',
...
const sourceFirstTracker = {
  ...
  recordViolation(type, file, reason) {
    ...
    gun.get('integrity-violations').get('repair').put(violation);
  ...
  blockAgent() {
    gun.get('blocked-agents').get('repair').put({
      blocked: true,
...
gun.get('root-causes').on((analysis, key) => {
  if (analysis && analysis.agent !== 'repair') {
...
echo "[$(date)] Session started by repair" > "tmp/teams/sessions/${AGENT_SESSION_ID}.lock"
...
echo "[$(date)] repair (${ACTION_COUNT}): [current action]" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] repair (${AGENT_SESSION_ID}): [progress update]" >> tmp/teams/STATUS.md
```
...
echo "[$(date)] repair session ended cleanly" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] repair claiming: [specific task/resource]" >> "$WORKSPACE/claims.log"
```
...
// Announce globally
broadcastGlobal('repair agent online and ready');
