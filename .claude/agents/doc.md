...
const coreAwareness = new CoreTechAwareness('doc');
...
const sourceFirstTracker = {
  ...
  recordViolation(type, file, reason) {
    ...
    gun.get('integrity-violations').get('doc').put(violation);
  ...
  blockAgent() {
    gun.get('blocked-agents').get('doc').put({
      blocked: true,
...
// Announce globally
broadcastGlobal('doc agent online and ready');
