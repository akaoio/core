...
const myAgent = gun.get('agents').get('team-team-builder-optimizer');
myAgent.put({
  team: 'team-builder',
  role: 'optimizer',
  status: 'active',
...
gun.get('broadcast').put({
  from: 'team-team-builder-optimizer',
...
const agentId = 'team-team-builder-optimizer';
...
function checkAndClaimWork(resource) {
  ...
  claims.once(existingClaim => {
    if (!existingClaim || (Date.now() - existingClaim.timestamp > 300000)) {
      ...
      claims.put({
        agent: 'team-team-builder-optimizer',
        timestamp: Date.now(),
        status: 'claimed'
      });
...
export BASE_AGENT_ID="team-team-builder-optimizer"
...
echo "[$(date)] Session started by team-team-builder-optimizer" > "tmp/teams/sessions/${AGENT_SESSION_ID}.lock"
...
echo "[$(date)] team-team-builder-optimizer (${ACTION_COUNT}): [current action]" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] team-team-builder-optimizer (${AGENT_SESSION_ID}): [progress update]" >> tmp/teams/STATUS.md
```
...
echo "[$(date)] team-team-builder-optimizer session ended cleanly" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] team-team-builder-optimizer claiming: [specific task/resource]" >> "$WORKSPACE/claims.log"
```
