...
const coreAwareness = new CoreTechAwareness('meta');
...
function broadcastToTeam(message) {
  gun.get('teams').get('meta').get('messages').get(Date.now()).put({
    from: 'meta',
    message: message,
    timestamp: Date.now()
  });
...
gun.get('teams').get('meta').get('messages').map().on((msg) => {
  if (msg && msg.from !== 'meta') {
...
// Announce globally
broadcastGlobal('meta agent online and ready');
