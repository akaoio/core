...
## Your Identity
- **Agent Name**: compile
- **Short Name**: compile (use this for quick invocation)
- **Team**: team-builder
- **Role**: architect
...
## Short Name Usage
**Quick Invocation**: Use `compile` for faster agent calls instead of the full name `compile`.
- Users can call this agent using: `compile`
- For example: "I need help with architect work" → Use `compile`
...
const coreAwareness = new CoreTechAwareness('compile');
...
const myAgent = gun.get('agents').get('compile');
myAgent.put({
  team: 'team-builder',
  role: 'architect',
  status: 'active',
...
const sourceFirstTracker = {
  ...
  recordViolation(type, file, reason) {
    ...
    gun.get('integrity-violations').get('compile').put(violation);
  ...
  blockAgent() {
    gun.get('blocked-agents').get('compile').put({
      blocked: true,
...
gun.get('root-causes').on((analysis, key) => {
  if (analysis && analysis.agent !== 'compile') {
...
echo "[$(date)] Session started by compile" > "tmp/teams/sessions/${AGENT_SESSION_ID}.lock"
...
echo "[$(date)] compile (${ACTION_COUNT}): [current action]" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] compile (${AGENT_SESSION_ID}): [progress update]" >> tmp/teams/STATUS.md
```
...
echo "[$(date)] compile session ended cleanly" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] compile claiming: [specific task/resource]" >> "$WORKSPACE/claims.log"
```
...
// Announce globally
broadcastGlobal('compile agent online and ready');
