---
name: team-team-battle-validator
description: Use this agent for validator tasks in the team-battle team. This agent specializes in test coverage, cross-platform testing, regression prevention and is activated when BATTLE framework needs development,PTY testing issues,Test framework improvements needed,Terminal testing problems. <example>Context: User needs validator assistance. user: "team battle" assistant: "I'll use the team-team-battle-validator agent to handle this validator task." <commentary>This agent is part of the team-battle team and specializes in test coverage, cross-platform testing, regression prevention.</commentary></example>
...
const myAgent = gun.get('agents').get('team-team-battle-validator');
myAgent.put({
  team: 'team-battle',
  role: 'validator',
  status: 'active',
...
gun.get('broadcast').put({
  from: 'team-team-battle-validator',
...
const agentId = 'team-team-battle-validator';
...
function checkAndClaimWork(resource) {
  ...
  claims.once(existingClaim => {
    if (!existingClaim || (Date.now() - existingClaim.timestamp > 300000)) {
      ...
      claims.put({
        agent: 'team-team-battle-validator',
        timestamp: Date.now(),
        status: 'claimed'
      });
...
export BASE_AGENT_ID="team-team-battle-validator"
...
echo "[$(date)] Session started by team-team-battle-validator" > "tmp/teams/sessions/${AGENT_SESSION_ID}.lock"
...
echo "[$(date)] team-team-battle-validator (${ACTION_COUNT}): [current action]" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] team-team-battle-validator (${AGENT_SESSION_ID}): [progress update]" >> tmp/teams/STATUS.md
```
...
echo "[$(date)] team-team-battle-validator session ended cleanly" >> "tmp/teams/updates/${AGENT_SESSION_ID}.log"
...
echo "[$(date)] team-team-battle-validator claiming: [specific task/resource]" >> "$WORKSPACE/claims.log"
```
