...
const coreAwareness = new CoreTechAwareness('bundle');
...
const sourceFirstTracker = {
  ...
  recordViolation(type, file, reason) {
    ...
    gun.get('integrity-violations').get('bundle').put(violation);
  ...
  blockAgent() {
    gun.get('blocked-agents').get('bundle').put({
      blocked: true,
...
// Announce globally
broadcastGlobal('bundle agent online and ready');
